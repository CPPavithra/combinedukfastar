...
Vector3d locate() {
    ...
    if (!csvFile.is_open()) {
        throw runtime_error("Error: Could not open simulation_data.csv");
    }

    static bool initialized = false;
    if (!initialized) {
        string temp;
        getline(csvFile, temp); // Skip header
        state_ << 10, 0, 0.5, 0, 0;
        initialized = true;
    }

    // 🔍 Reset file stream if EOF is reached
    if (!csvFile.good()) {
        csvFile.clear();
        csvFile.seekg(0, ios::beg);
        string temp;
        getline(csvFile, temp); // Skip header again
    }

    double gps_x, gps_y, gps_yaw, imu_ax, imu_yaw_rate, additional_ax, additional_yaw_rate;
    if (!readCSVData(csvFile, gps_x, gps_y, gps_yaw, imu_ax, imu_yaw_rate, additional_ax, additional_yaw_rate)) {
        cerr << "DEBUG: readCSVData failed, file might be empty or corrupted." << endl;
        throw runtime_error("No more data available in the CSV file.");
    }
